 ...
 */
/*
== 중요한건 소스의 단순화 소수량의 감소로 인해 개발에 탄력이 붙을 수 있도록 함 ==
- frozen row, frowzen columns : 여기서 막혔지.
- data를 만들고 그냥 뿌려주는 방식으로 하자.
- 크게 고치지 말자. 영역만 바로 잡아줘도 괜찮을듯. content, left, right 사용하지 말고 panel 숫자로 할 것.
- 영역마다 관리하도록 함
 */
...
TbsGrid.prototype.tbs_displayPanel30_merge = function (topRowIndex) {
    ...
    //=============================================================
    if (document.querySelector(selector + ' .tbs-grid-panel21 td div').textContent != this.data_panel30.length) //행의 total
...
TbsGrid.prototype.tbs_displaySelectedLineFixCol = function() {
    ...
    let rowArray = [];
    let colArray = [];
    //============================================================= find view area / 해당 rowArray와 colArray이가 화면 존재여부
    ...
    //============================================================= fixBottom, content active 여부
    let activefixBottom = false; //fixBottom에 selectColIndex 여부
    let activeContent   = false; //content에 selectColIndex 여부
    ...
    if (activefixBottom && activeContent) {
        ...
        //멀티 헤더일 경우 -> startCellRect 를 가질 수 있는 cell을 찾는다.
        ...
        let startCellTop  = leftCellRect.top   - gridRect.top  - 1;		//left로 구할 수 있음
        let startCellLeft = startCellRect.left - gridRect.left - 1; 	//header로 구할 수 있음

        //============================================================= option
        if (colArray[colArray.length - 1] == lastColIndex) { //header로 구할 수 있음...
        ...
        //============================================================= line drawing
        if (width <= 3 || height <= 3) {
            this.topLineDiv.style    = 'width:0px;top:0px;left:0px;';	 //topLineDiv(absolute) left tbs-grid(relative) 위치를 기준으로 하는구나
            this.leftLineDiv.style   = 'height:0px;top:0px;left:0px;';
            this.bottomLineDiv.style = 'width:0px;top:0px;left:0px;';
            this.rightLineDiv.style  = 'height:0px;top:0px;left:0px;';
        }
    ...
    //============================================================= else if (activeContent) {
        ...
        if (colArray[0] == startCell.cellIndex) {
            if (startCellRect.left < contentRect.left) { //header로 구할 수 있음...
...
TbsGrid.prototype.tbs_displaySelectedLineFixRow = function() {
    ...
    //============================================================= fixBottom, content active 여부
    let activeFixRight  = false; //activeFixRight selectColIndex 여부
    let activeContent   = false; //content에 selectColIndex 여부
    for (let i = 0; i <= this.fixedRowIndex; i++){

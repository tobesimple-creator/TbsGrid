/**
 * [tobesimple grid] tbs.grid.2.0.0.event.js
 *
...
TbsGrid.prototype.event_mobileTouchDrag = function() { //type : header, content
	...
	const touchMoveEvent = function(e) {	
        ...
        if (Math.abs(xMove) > Math.abs(yMove)) {
            ...
            let ratio =  (header.childNodes[0].clientWidth - header.clientWidth) / xRailWidth; //rail 길이 대비 table 길이
...
TbsGrid.prototype.event_dragScrollBar = function() {
	...
	const xMouseDownEvent = function(e) {
		...
		e.stopPropagation(); //전파방지하는데 약간의 시간이 필요한듯 불필요하게 쓰지 않도록 함
	...
	const xMouseMoveEvent = function(e) {
		//e.preventDefault();		//element 동작 방지
		//e.stopPropagation();	//상위 element 동작도 방지
...
TbsGrid.prototype.tbs_moveCell = function (type) { //type : left, right, up, down
    ...
    if (type == 'left') {
        ...
        //cellIndex에 해당하는 cell의 절대값 right1
        let tableRow = tableRows[tableRowIndex - 1];
        ...
        if (this.pageRowCount > this.pageIntRowCount) {
            if (tableRows.length == this.pageRowCount) { //table에 데이타가 찼을 경우
                if (dataRowIndex == lastRowIndex) { //table의 마지막행일 경우
                    if (dataRowIndex == this.data_panel30.length - 1) { //마지막 데이타 일 경우
                        this.tbs_clearRange(0, -1);
                ...
                else if (dataRowIndex == lastRowIndex - 1) { //table의 마지막행 앞일 경우
                    dataRowIndex += 1;
                    this.tbs_clearRange(0, -1);
                    this.tbs_setBarPositionByDirection('down', topRowIndex + 1);
                    this.tbs_displayOneSelection(dataRowIndex, dataRowIndex, cellIndex, cellIndex);
        ...
        //==========================================================================
            ...
            if (tableRows.length == this.pageRowCount) { //table에 데이타가 찼을 경우
                if (dataRowIndex == lastRowIndex) { //table의 마지막행일 경우
                    if (dataRowIndex == this.data_panel30.length - 1) { //마지막 데이타 일 경우.. 변화 없음
                        this.tbs_clearRange(0, -1);
                        this.tbs_selectedRange(dataRowIndex, dataRowIndex, cellIndex, cellIndex);
                    }
                    else { //마지막 데이타가 아닐 경우

...
TbsGrid.prototype.event_input = function() {
    ...
    const keydownEvent = function (e) {
        ...
        let input = document.querySelector(selector + ' .tbs-grid-input');
        let mode = input.dataset.mode; //이제 의미가 없음. userfunction을 함수로 전달하므로.
        if (mode == undefined) mode = '';
        if (e.ctrlKey) {
            if      (e.keyCode == 37 && mode == '') { if(!(grid.options[grid.option_addRow])) return; grid.input_blur(); grid.tbs_addRow('up');   grid.input_focus();}   //왼쪽 화살표 //type : first, last, up, down
            else if (e.keyCode == 39 && mode == '') { if(!(grid.options[grid.option_addRow])) return; grid.input_blur(); grid.tbs_addRow('down'); grid.input_focus();}   //오른쪽 화살표
            else if (e.keyCode == 38 && mode == '') { if(!(grid.options[grid.option_addRow])) return; grid.input_blur(); grid.tbs_addRow('first');grid.input_focus();}   //위 화살표
            else if (e.keyCode == 40 && mode == '') { if(!(grid.options[grid.option_addRow])) return; grid.input_blur(); grid.tbs_addRow('last'); grid.input_focus();}   //아래 화살표
            ...
            else if (e.keyCode >= 0   && e.keyCode <= 7)  {} //특수키 : function ctrlkey shifkey tabkey(9)등등
    ...
    const blurEvent = function(e) {
        //blur event 보다 focus event 가 더 먼저 나옴
        //console.dir(e);
    ...
    const copyEvent = function(e){
        // * startRowIndex, startCellIndex부터 select를 검색해서....다 가져오기
    ...
    const pasteEvent = function(e){
        //111\t111\r\n
        //222\t222
        //startRowIndex, startCellIndex는 화면에서 보여야함
        // * startRowIndex와 startCellIndex에 해당 하는 td를 찾아야 함...
        // * view, center data update
        ...
        for (let rowIndex = startRowIndex; rowIndex < startRowIndex + rowArray.length; rowIndex++) {
            if (rowIndex >= grid.data_panel30.length) break;
    ...
    input.addEventListener('keydown', keydownEvent, true); //div에 tabindex가 있어야 함
...
/**
 *
 * input icon click : 레이어 정보가 뜰거고, input_show 호출 필요
 *
 */
TbsGrid.prototype.event_input_icon = function() {
    ...
    //console.log(`아이콘 클릭 이벤트 세팅`);
    const mousedownEvent = function(e) {
        //console.dir(e)
        //console.log(`mousedownEvent 클릭`);
        e.preventDefault();
        e.stopPropagation();
        e.stopImmediatePropagation();
...
/**
 *
 * input_show : dblclick, keyup 일 경우 발생
 *
 * @param e : event
...
TbsGrid.prototype.input_show = function(e, mode) { //type : header, content, left, top
    ...
    let value  = this.tbs_getValue(rowIndex, column[grid.column_id]);
    let result = this.tbs_render(column, value); //result.value, result.text

    if (column[grid.column_editable] == false) return;
    //if (columns[cellIndex].column_autosize == true) return;
    //if (grid.data_panel30[rowIndex].mode != 'I') return;

    grid.tbs_moveCellLine(td, rowIndex, cellIndex);

    let left = td.getBoundingClientRect().left;    // input 은 viewport에 맞춰줌(절대위치) input은 relative를 타지 않는듯함.
...
/**
 *
 * input_hide : enter, tab focus out 일 경우 발생
 *
 */
...
TbsGrid.prototype.input_edit = function(e, state, mode) {
    ...
    if (column[grid.column_editable] == true && grid.user_edit != '') {
        let result = true;
        if (state == 0) {

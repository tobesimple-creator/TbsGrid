...
TbsGrid.prototype.tbs_setBarPosition = function (type, value) {
	...
	//
	// @type  : const_horizontal, this.const_vertical
...
TbsGrid.prototype.tbs_setBarPositionByDirection = function(type, rowIndex) {
	...
	if (type == 'up') {
		...
		//-------------------------------------------------------------
		this.tbs_setBarPosition(this.const_vertical, displayTopRowIndex);
	...
	if (type == 'down') {
		...
		if (this.pageRowCount > this.pageIntRowCount) {
			if (trTopIndex > (this.data_panel30.length - this.pageRowCount + 1))	return false;
		}
		else {
			if (trTopIndex > (this.data_panel30.length - this.pageRowCount)) 	return false;
		}
		//-------------------------------------------------------------
		this.tbs_setBarPosition(this.const_vertical, displayTopRowIndex);
		this.tbs_displayPanel30(trTopIndex)
...
TbsGrid.prototype.tbs_getContentPanelLeft = function (value) {
    ...
    let railSize = this.scroll.xRailSize;
    let ratio = (panel20.childNodes[0].clientWidth - panel20.clientWidth) / railSize; //rail 길이 대비 table 길이
...
TbsGrid.prototype.tbs_setAllColumnWidth = function (arr) {
	...
	for (let x = 0, len = this.columns.length; x < len; x++){
		this.columns[x].width = arr[x];
		let nWidth = arr[x] + 'px';
		thList20[x].style.width = nWidth;
		thList30[x].style.width = nWidth;
		if (this.topColumns.length    > 0) thList40[x].style.width = nWidth;
		if (this.footerColumns.length > 0) thList50[x].style.width = nWidth;
